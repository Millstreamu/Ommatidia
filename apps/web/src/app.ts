...
export function renderFixtureList(fixtures: UiFixture[], options: { loading?: boolean; error?: string } = {}): string {
  if (options.loading) return '<p>Loading fixtures...</p>';
  if (options.error) return `<p style="color:#b91c1c;">Could not load fixtures: ${options.error}</p>`;
  if (!fixtures.length) return '<p>No fixtures saved yet.</p>';
  return `<ul>${fixtures.map((f) => `<li><strong>${f.name}</strong> • ${f.originalFilename} • ${f.candidateValues.length} values • Component: ${f.componentName ?? 'Unassigned'} • ${new Date(f.createdAt).toLocaleString()}</li>`).join('')}</ul>`;
}
...
export function mountApp(root: HTMLElement, apiBaseUrl: string): void {
  ...
  const load = async () => {
    ...
    if (hash.startsWith('#/projects/')) {
      ...
      <section><p>${project.description ?? 'No description provided.'}</p><div><strong>Quick stats:</strong> Components ${components.length} • Documents ${documents.length} • Values needing review ${needsReviewCount} • Approved values ${approvedCount} • Fixtures count <span id="fixture-count">-</span></div></section>
      <div style="display:grid;gap:16px;grid-template-columns:repeat(auto-fit,minmax(420px,1fr));">
      <section><h3>Components</h3><p>Select the component you are currently reviewing.</p><ul>${components.map((c) => `<li>${c.name} (${c.type})</li>`).join('')}</ul><form id="component-form"><input name="name" placeholder="Component name" required/><input name="type" placeholder="Component type" required/><button>Add component</button></form></section>
      <section><h3>Selected component data</h3>${valuesSection}</section>
      <section><h3>Documents</h3><p>Supported upload file types: PDF, PNG, JPG, JPEG, WEBP.</p><form id="document-form"><input name="file" type="file" accept=".pdf,.png,.jpg,.jpeg,.webp" required/><select name="documentType"><option value="datasheet">datasheet</option><option value="manual">manual</option><option value="quote">quote</option><option value="schematic">schematic</option><option value="drawing">drawing</option><option value="other">other</option></select><button>Upload document</button></form><ul>${documents.map((d)=>`<li>${d.originalFilename} | ${d.documentType} | Component: <select data-document-component-id="${d.id}"><option value="">Unassigned</option>${components.map((c)=>`<option value="${c.id}" ${d.componentId===c.id?'selected':''}>${c.name}</option>`).join('')}</select> | Mode: ${latestSystemStatus?.extractionProvider ?? 'unknown'} | Fixture: <select data-extract-fixture-id="${d.id}"><option value="">Select fixture</option></select> | <button data-extract-doc-id="${d.id}">Run extraction</button> <button data-save-fixture-doc-id="${d.id}">Save fixture</button> <button data-retry-doc-id="${d.id}">Retry</button> <span id="extract-status-${d.id}"></span><ul id="extract-attempts-${d.id}"></ul></li>`).join('')}</ul></section>
      <section><h3>Extraction fixtures</h3><p>Select fixture → selected component → replay fixture.</p><div id="fixture-list">Loading fixtures...</div></section>
      ...
      try {
        ...
        const fixtureCountEl = view.querySelector('#fixture-count') as HTMLElement | null;
        if (fixtureCountEl) fixtureCountEl.textContent = String(availableFixtures.length);
      } catch (error) {
        if (fixtureListEl) fixtureListEl.innerHTML = renderFixtureList([], { error: (error as Error).message });
        const fixtureCountEl = view.querySelector('#fixture-count') as HTMLElement | null;
        if (fixtureCountEl) fixtureCountEl.textContent = '0';
      }
