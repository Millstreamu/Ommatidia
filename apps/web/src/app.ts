...
export function renderExtractionDiagnostics(attempt: { diagnostics?: Record<string, unknown>; warnings?: string[] }): string {
  ...
  return `Text extracted: ${extracted > 0 ? 'yes' : 'no'} • chars: ${extracted} • useful chars: ${useful} • OpenAI called: ${called}${warning ? ` • warning: ${warning}` : ''}`;
}
...
export function mountApp(root: HTMLElement, apiBaseUrl: string): void {
  ...
  const load = async () => {
    ...
    if (hash.startsWith('#/projects/')) {
      ...
      for (const d of documents) { const attempts = await client.listExtractionAttempts(projectId, d.id); const el = view.querySelector(`#extract-attempts-${d.id}`) as HTMLElement | null; if (el) el.innerHTML = attempts.map((a) => { const droppedSummary = renderDroppedCandidateWarnings(a); const diag = renderExtractionDiagnostics(a); return `<li><strong>${a.status}</strong> | provider: ${a.provider} | created: ${a.valuesCreatedCount}${a.valuesCreatedCount===0?' (zero values)':''} | ${a.errorCode ?? 'no error'}${a.warnings?.length?`<br/><span style="color:#92400e;">${a.warnings.join(' | ')}</span>`:''}${diag?`<br/><span style="color:#334155;">${diag}</span>`:''}${droppedSummary?`<br/><span style="color:#92400e;">${droppedSummary}</span>`:''}${a.safeErrorMessage?`<br/><span style="color:#b91c1c;">${a.safeErrorMessage}</span>`:''}</li>`; }).join(''); }
