...
const OPERATOR_OUTCOME_LABELS: Record<string, string> = {
  stood_aside: 'Stood aside (no trade taken)',
  acted_no_fill: 'Acted, no fill',
  acted_opened: 'Acted, opened position',
  acted_round_trip: 'Acted, round trip complete'
};

export function formatOperatorOutcome(outcome: string): string {
  return OPERATOR_OUTCOME_LABELS[outcome] ?? outcome.replaceAll('_', ' ');
}
...
export function renderEngineeringValuesSection(components: UiComponent[], values: UiEngineeringValue[], promotingComponentId?: string): string {
  ...
  const cards = components.map((component) => {
    ...
    const promoteForm = promotingComponentId === component.id ? `<form data-promote-form-id="${component.id}" style="margin:8px 0;padding:8px;border:1px solid #cbd5e1;border-radius:8px;">
      ...
      ${promoteCount === 0 ? '<p class="alert alert-warning">No approved/user-entered values are available to promote.</p>' : ''}
...
export function renderComponentLibrarySection(library: Array<{ id: string; name: string; componentType: string; tags: string[]; approvedEngineeringValues: Array<{ key: string; value: unknown }>; originatingProjectId?: string; originatingComponentId?: string }>, options: { search?: string; error?: string; copyingLibraryId?: string; copySupported?: boolean } = {}): string {
  ...
  return `<ul>${library.map((i) => {
    ...
    const copyForm = options.copyingLibraryId === i.id
      ? `<form data-library-copy-form-id="${i.id}" style="margin-top:8px;padding:8px;border:1px solid #cbd5e1;border-radius:8px;">
          <label>Component name override: <input name="componentName" value="${escapeHtml(i.name)}" /></label><br/>
          <button type="submit">Confirm copy</button>
          <button type="button" data-library-copy-cancel-id="${i.id}">Cancel</button>
        </form>`
      : '';
    return `<li><strong>${i.name}</strong><br/>Type: ${i.componentType}<br/>Tags: ${(i.tags.length ? i.tags.map((tag) => `<span class="badge badge-secondary">${escapeHtml(tag)}</span>`).join(' ') : 'none')}<br/>Approved/user-entered values: ${i.approvedEngineeringValues.length}<br/>Origin: ${i.originatingProjectId ?? 'n/a'} / ${i.originatingComponentId ?? 'n/a'}${manufacturer || model ? `<br/>Manufacturer/Model: ${String(manufacturer ?? 'n/a')} / ${String(model ?? 'n/a')}` : ''}<br/>${options.copySupported ? `<button data-library-copy-id="${i.id}">Copy to current project</button>` : ''}${copyForm}</li>`;
  }).join('')}</ul>`;
}
...
export function mountApp(root: HTMLElement, apiBaseUrl: string): void {
  ...
  const load = async () => {
    ...
    if (hash.startsWith('#/projects/')) {
      ...
      const justPromoted = new URLSearchParams(window.location.search).get('promoted') === '1';
      const copiedComponentId = new URLSearchParams(window.location.search).get('copiedComponentId') ?? '';
      ...
      if (isDocumentRoute) {
        ...
        const copySuccess = copiedComponentId ? `<div class="alert alert-success">Library component copied to project. <a href="#/projects/${projectId}/components/${copiedComponentId}">Open new component</a></div>` : '';
        view.innerHTML = `${copySuccess}${renderComponentLibraryPageShell(projectId, '<p>Loading component library...</p>')}`;
      ...
      for (const d of documents) {
        ...
        if (summaryEl) {
          ...
          summaryEl.textContent = latest ? `${formatOperatorOutcome(latest.status)} • provider ${latest.provider} • created ${latest.valuesCreatedCount}` : 'No attempts yet';
        }
      }
      if (isDocumentRoute) {
      ...
      const latestOutcomeEl = view.querySelector('#latest-session-outcome') as HTMLElement | null;
      if (latestOutcomeEl && documents.length) {
        let latestAttempt: { status: string; completedAt?: string; startedAt?: string } | undefined;
        for (const document of documents) {
          const attempts = await client.listExtractionAttempts(projectId, document.id);
          const newest = attempts[0];
          if (!newest) continue;
          const newestTime = new Date(newest.completedAt ?? newest.startedAt ?? 0).getTime();
          const latestTime = latestAttempt ? new Date(latestAttempt.completedAt ?? latestAttempt.startedAt ?? 0).getTime() : -1;
          if (newestTime >= latestTime) latestAttempt = newest;
        }
        latestOutcomeEl.textContent = latestAttempt ? formatOperatorOutcome(latestAttempt.status) : 'No attempts yet';
      }
      ...
      view.querySelectorAll<HTMLFormElement>('form[data-promote-form-id]').forEach((form) => { form.onsubmit = async (e) => { e.preventDefault(); const fd = new FormData(form); const componentId = form.dataset.promoteFormId!; try { await client.promoteComponentToLibrary({ projectId, componentId, name: String(fd.get('name') ?? ''), tags: String(fd.get('tags') ?? '').split(',').map((t) => t.trim()).filter(Boolean), description: String(fd.get('description') ?? '') || undefined }); const url = new URL(window.location.href); url.searchParams.delete('promote'); url.searchParams.set('promoted', '1'); window.history.replaceState({}, '', url); await load(); } catch (error) { alert(`Could not promote component: ${(error as Error).message}`); } }; });
      ...
      const loadLibrary = async (search = '') => {
        ...
        const copyingLibraryId = params.get('copy') ?? undefined;
        if (isLibraryRoute) {
        ...
        try {
          ...
          if (isLibraryRoute) {
            const listEl = view.querySelector('#component-library-list') as HTMLElement | null;
            if (listEl) listEl.innerHTML = renderComponentLibrarySection(list, { search, copyingLibraryId, copySupported: typeof client.copyLibraryToProject === 'function' });
          } else {
      ...
      await loadLibrary('');
      view.querySelectorAll<HTMLButtonElement>('button[data-library-copy-id]').forEach((btn) => { btn.onclick = async () => { const url = new URL(window.location.href); url.searchParams.set('copy', btn.dataset.libraryCopyId!); window.history.replaceState({}, '', url); await load(); }; });
      view.querySelectorAll<HTMLButtonElement>('button[data-library-copy-cancel-id]').forEach((btn) => { btn.onclick = async () => { const url = new URL(window.location.href); url.searchParams.delete('copy'); window.history.replaceState({}, '', url); await load(); }; });
      view.querySelectorAll<HTMLFormElement>('form[data-library-copy-form-id]').forEach((form) => { form.onsubmit = async (e) => { e.preventDefault(); const libraryId = form.dataset.libraryCopyFormId!; const componentName = String(new FormData(form).get('componentName') ?? '').trim() || undefined; const result = await client.copyLibraryToProject(libraryId, { targetProjectId: projectId, componentName }); const url = new URL(window.location.href); url.searchParams.delete('copy'); url.searchParams.set('copiedComponentId', result.component.id); window.history.replaceState({}, '', url); await load(); }; });
