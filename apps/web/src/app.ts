...
export function renderExtractionDiagnostics(attempt: { diagnostics?: Record<string, unknown>; warnings?: string[] }): string {
  ...
  return `Text extracted: ${extracted > 0 ? 'yes' : 'no'} • chars: ${extracted} • useful chars: ${useful} • suspicious/internal ratio: ${suspiciousRatio.toFixed(2)} • OpenAI called: ${called}${warning ? ` • warning: ${warning}` : ''}`;
}
