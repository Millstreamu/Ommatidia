...
export function renderDocumentDetailView(options: { projectId: string; document: { id: string; originalFilename: string; documentType: string; fileSizeBytes: number; uploadStatus: string; processingStatus: string; componentId?: string }; componentName?: string; apiBaseUrl: string }): string {
  const { projectId, document, componentName, apiBaseUrl } = options;
  return `<p><a href="#/projects/${projectId}">Back to project</a></p><h2>Document detail</h2><p><strong>Filename:</strong> ${escapeHtml(document.originalFilename)}</p><p><strong>Document type:</strong> ${escapeHtml(document.documentType)}</p><p><strong>File size:</strong> ${document.fileSizeBytes} bytes</p><p><strong>Status:</strong> ${escapeHtml(document.uploadStatus)} / ${escapeHtml(document.processingStatus)}</p><p><strong>Assigned component:</strong> ${escapeHtml(componentName ?? 'Unassigned')}</p><p><a href="${apiBaseUrl}/documents/${document.id}/file" target="_blank" rel="noreferrer">View file</a></p>`;
}
